...
class ZkStars(Account):
    ...
    async def mint(
        ...
        contracts = (
            contracts if mint_all is True else random.sample(contracts, quantity_mint)
        )

        logger.info(
            f"[{self.account_id}][{self.address}] Mint {quantity_mint} ZKStars NFT"
        )

        try:
            for _, contract in enumerate(contracts, start=1):
                mint_contract = self.get_contract(
                    self.w3.to_checksum_address(contract), ZKSTARS_ABI
                )
                ...
                transaction = await mint_contract.functions.safeMint(
                    self.w3.to_checksum_address(
                        "0x1C7FF320aE4327784B464eeD07714581643B36A7"
                    )
            ...
            logger.error(
                f"[{self.account_id}][{self.address}] Mint ZKStars Error | {e}"
            )
            raise e
